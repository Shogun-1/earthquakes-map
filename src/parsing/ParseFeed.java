...
public class ParseFeed {
	...
	// to get rid of " symbols
	private static String parsesHelper(String input) {
	...
	public static List<PointFeature> parseAirports(PApplet p, String fileName) {
		...
		for (String row : rows) {
			...
			point.addProperty("name", parsesHelper(columns[1]));
			point.putProperty("city", parsesHelper(columns[2]));
			point.putProperty("country", parsesHelper(columns[3]));
			
			// pretty sure IATA/FAA is used in routes.dat
			// get airport IATA/FAA code
			if(!columns[4].equals("")) {
				point.putProperty("code", parsesHelper(columns[4]));
			}
			// get airport ICAO code if no IATA
			else if(!columns[5].equals("")) {
				point.putProperty("code", parsesHelper(columns[5]));
			}
			
			point.putProperty("altitude", columns[8 + i]);

			if (!columns[11].equals("\\N")) {
				String[] data = columns[11].split("/");
				point.addProperty("region", parsesHelper(data[0]));
			}
